...
public sealed class LocalizationService
{
    private readonly Dictionary<string, Dictionary<string, string>> _resources = new(StringComparer.OrdinalIgnoreCase)
    {
        ["zh-TW"] = new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["UpdateStatusRepoHintTemplate"] = "（來源：{0}/{1}）",
            ["UpdateStatusNoRelease"] = "找不到已發佈的版本。請確認 GitHub 上的 Release 已按「Publish release」正式發佈（草稿不會出現在清單），並確認儲存庫設定正確；亦可按「重新整理版本」。",
            ["UpdateStatusLoadedTemplate"] = "已載入 {0} 個版本。",
            ["UpdateStatusLoadFailedTemplate"] = "讀取版本失敗: {0}",
            ["UpdateStatus404HintTemplate"] = "GitHub 回傳 404：找不到儲存庫「{0}/{1}」，或此 Repo 為私人（匿名 API 也會顯示 404）。請在瀏覽器確認網址 https://github.com/{0}/{1} 是否存在且為公開；若不是這個 Repo，請設定環境變數 CSARCHVIEWER_UPDATE_REPO=擁有者/Repo名稱 後重新開啟程式。\n請求：{2}",
            ["UpdateStatusApiUrlTemplate"] = "請求：{0}",
        ...
        ["en-US"] = new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["UpdateStatusRepoHintTemplate"] = "(source: {0}/{1})",
            ["UpdateStatusNoRelease"] = "No published releases found. Make sure the GitHub release is published (drafts are hidden), the repo settings match this app, then tap Refresh.",
            ["UpdateStatusLoadedTemplate"] = "Loaded {0} versions.",
            ["UpdateStatusLoadFailedTemplate"] = "Failed to load versions: {0}",
            ["UpdateStatus404HintTemplate"] = "GitHub returned 404: repo \"{0}/{1}\" was not found, or the repo is private (anonymous API also returns 404). Verify https://github.com/{0}/{1} exists and is public. If yours differs, set env var CSARCHVIEWER_UPDATE_REPO=owner/repo and restart.\nRequest: {2}",
            ["UpdateStatusApiUrlTemplate"] = "Request: {0}",
